...
## Narrative Angles Integration

The three narrative sections (`narrativeDistributor`, `narrativeProcurement`, `narrativeSku`)
are generated by Claude using a **deterministic angle** selected from the submission's form inputs
before any AI call is made. The angle shapes the tone, emphasis, and talking points for each section.
...
| `gpo_member` | GPO (legacy — no longer a form option) | Sophisticated |
...
| `optimized_buyer` | negotiated_cost_plus | Analytical, mature |

Example: A restaurant buying from Sysco at market price receives the **Captive Buyer** angle —
"Your distributor is setting your food cost — you are not." The PDF narrative emphasizes
...
- `docs/ai-narrative.md` — narrative fields, angle selection, prompt structure
